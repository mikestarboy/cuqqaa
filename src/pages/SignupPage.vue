    ...
    <div class="container">
      ...
      <div class="auth-grid">
        <div class="auth-panel">
          <span class="section-tag">{{ content.nav.signup }}</span>
          <h1 class="auth-title">{{ content.signup.title }}</h1>
          <p class="auth-subtitle">{{ content.signup.subtitle }}</p>
          <form class="auth-form" @submit.prevent>
            <label class="auth-field">
              <span>{{ content.signup.name }}</span>
            ...
            <label class="auth-field">
              <span>{{ content.signup.email }}</span>
              <div class="auth-field__control">
            ...
            <label class="auth-field">
              <span>{{ content.signup.password }}</span>
              <div class="auth-field__control">
            ...
            <label class="auth-checkbox">
              ...
              <span>{{ content.signup.agreement }}</span>
            </label>
            <button class="btn btn-primary btn-primary--lift" type="submit">
              <i class="fa-solid fa-user-plus"></i>
              {{ content.signup.primaryCta }}
            </button>
          </form>
          <RouterLink class="auth-switch" to="/login">
            {{ content.signup.secondaryCta }}
        ...
        <div class="auth-aside" aria-hidden="true">
          ...
          <div class="auth-aside__card">
            <span>{{ content.signup.aside.label }}</span>
            <strong>{{ content.signup.aside.value }}</strong>
            <small>{{ content.signup.aside.helper }}</small>
          </div>
          <div class="auth-aside__stat">
            <i class="fa-solid fa-bolt"></i>
            {{ content.signup.aside.stat }}
          </div>
        </div>
      </div>
    </div>
